...
def download_video_from_url(url):
    ...
    return {'title': title, 'url': url_play_list}
...
def get_real_url_video_from_series(id):
    ...
    for item in id_videos:
        ...
        if check_exist_chapt(id, video_id):
            results.append(link)
...
if __name__ == '__main__':
    ...
    for item in datas:
        ...
        title = title.replace(" ", "-")
        os.system(
            "youtube-dl --console-title --hls-prefer-native --hls-use-mpegts -c --no-part --fixup never \"" + url_real + "\" -o output/" + title + ".mp4")

        save_to_file(id, video_id)
