...
def handle(id_series, stt_id):
    ...
    if arr != False:
        stt = get_data_file(stt_id, "stt").replace("\n", '')
        title = str(arr['title'])
        description = str(get_description(arr['id_video'])) + str("\n Content #") + str(stt)
        ...
        os.system("youtube-dl --console-title --hls-prefer-native --hls-use-mpegts -c --no-part --fixup never \"" + result + "\" -o " + str(stt_id) + '/temp.mp4')

        string_ts = "ffmpeg -i " + str(stt_id) + '/temp.mp4' + " -c copy -bsf:v h264_mp4toannexb -f mpegts " + str(stt_id) + '/input.ts'
        os.system(string_ts)

        file_name = str(stt_id) + '/input.ts'

        file_video = str(stt_id) + "/videos/" + str(stt) + ".ts"

        # string_concat = 'ffmpeg -i concat:"' + file_name + '|' + file_video + '" -c copy -bsf:a aac_adtstoasc ' + str(
        #     stt_id) + '/output.mp4'
        #
        ...
        os.system(string_concat)
        #
        # os.system('ffmpeg -i ' + str(stt_id) + '/output.ts -c:v libx264 -c:a copy ' + str(stt_id) + '/output.mp4')
        ...
        if int(stt_id) == 1 or int(stt_id) == 3 or int(stt_id) == 5:
            print("upload to fb...")
            #id_page = "me"
            #access_token = get_data_file('page_access_token.txt', 3)
        ...
        print('Done!')
        stt = int(stt) + 1
        max_video = get_data_file(stt_id, "max-video")

        if stt > int(max_video):
            stt = 1

        update_stt_video(stt_id, stt)
...
def convert_ts():
    for i in range(3, 4):
        ...
        for file in list_file_delete_1:
            ...
            string = "ffmpeg -i '" + item + "' -c copy -bsf:v h264_mp4toannexb -f mpegts " + str(i) + "/videos/" + str(stt) + ".ts"
